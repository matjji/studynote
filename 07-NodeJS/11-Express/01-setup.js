...
app.use((req, res, next) => {
  ...
  //클라이언트의 접속이 종료된 경우의 이벤트 --> 모든 응답의 전송이 완료도니 경우
  res.on('finish', () => {
    // 접속 종료시간
...
/** 쿠키를 처리할 수 있는 객체 연결 */
// 04 cookie-parser는 데이터를 저장, 조회 할 때 암호화 처리를 동반한다.
...
// public/02_get_params_by_from.html
...
router.get('/send_get', (req, res, next) => {
  ...
  //  /send_get?answer=0000 형태로 접근할경우 answer파라미터 값 추출
  //const answer = req.qeury['answer'];
...
router.get('/send_url/:username/:age', (req, res, next) => {
  // URL 파라미터들은 req.parmas 객체의 하위 데이터로 저장된다.
...
router.delete('/send_delete', (req, res, next) => {
  // URL 파라미터들은 req.body 객체의 하위 데이터로 저장된다.
  logger.debug('[프론트엔드로부터 전달받은 DELETE파라미터');
  for (let key in req.body) {
    const str = '\t >> ' + key + '=' + req.body[key];
    logger.debug(str);
  }
...
router
  ...
  .post('/product', (req, res, next) => {
    // <from> 상에 저장할 정보를 입력 후 전송한다.(주로 관리자 기능)
