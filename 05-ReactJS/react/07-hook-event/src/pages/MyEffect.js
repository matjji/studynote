...
import img from '../KakaoTalk_20220401_110519154.jpg'
...
const MyEffect = () => {
    ...
    React.useEffect(() => {
        console.log('[MyEffetc2] %s ::: 화면에 컴포넌트가 처음 로드되거나 state, props 중 하나라도 변경될 경우 호출됨', new Date());
    });

    /** 이 컴포넌트가 호면에 막 등장할 때와 특정 state, props값이 변경될 때만 실행됨 */
    React.useEffect(() => {
